...
public class TreeBreedingSystem extends BreedingSystem {
	...
	public float getChance(final IMutation mutation, final EntityPlayer player, final IAlleleSpecies firstSpecies, final IAlleleSpecies secondSpecies) {
		...
		final ITreeGenome genome0 = (ITreeGenome) speciesRoot.templateAsGenome(speciesRoot.getTemplate(firstSpecies.getUID()));
		final ITreeGenome genome2 = (ITreeGenome) speciesRoot.templateAsGenome(speciesRoot.getTemplate(secondSpecies.getUID()));
		return ((ITreeMutation) mutation).getChance(player.worldObj, player.getPosition(), (IAlleleTreeSpecies) firstSpecies, (IAlleleTreeSpecies) secondSpecies, genome0, genome2);
	}
	...
	public Collection<IAlleleSpecies> getTreesThatMakePlanks(final ItemStack fruit, final boolean nei, final World world, final GameProfile player) {
		...
		for (final IAlleleSpecies species : set) {
			...
			if (plankProduct != null && fruit.isItemEqual(plankProduct)) {
				found.add(species);
			}
		}
		return found;
