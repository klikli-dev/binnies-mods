...
public enum ExtraBeesFlowers implements IFlowerProvider, IAlleleFlowers {
	...
	public ItemStack[] affectProducts(World world, IIndividual individual, BlockPos pos, ItemStack[] products) {
		if (this == ExtraBeesFlowers.MYSTICAL) {
			final List<ItemStack> finalProducts = new ArrayList<>();
			Collections.addAll(finalProducts, products);
			for (int k = 0; k < 50; ++k) {
				final int tX = 7;
				final int tY = 7;
				final int tZ = 3;
				...
				if (block == Utils.getBotaniaBlock("flower")) {
					...
					if (item != null) {
						finalProducts.add(new ItemStack(item, 1, meta));
					}
				}
			}
			return (ItemStack[])finalProducts.toArray();
		}
		return products;
	}
