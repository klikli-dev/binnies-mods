...
public enum ExtraBeesEffect implements IAlleleBeeEffect {
	...
	SPAWN_ZOMBIE{
		...
		protected IEffectData doEffect(IBeeGenome genome, IEffectData storedData, IBeeHousing housing, World world, BlockPos position) {
			if (world.rand.nextInt(200) < 2) {
				this.spawnMob(world, position, "Zombie");
			}
			return storedData;
		}
	},
	SPAWN_SKELETON{
		...
		protected IEffectData doEffect(IBeeGenome genome, IEffectData storedData, IBeeHousing housing, World world, BlockPos position) {
			if (world.rand.nextInt(200) < 2) {
				this.spawnMob(world, position, "Skeleton");
			}
			return storedData;
		}
	},
	SPAWN_CREEPER{
		...
		protected IEffectData doEffect(IBeeGenome genome, IEffectData storedData, IBeeHousing housing, World world, BlockPos position) {
			if (world.rand.nextInt(200) < 2) {
				this.spawnMob(world, position, "Creeper");
			}
			return storedData;
		}
	},
	...
	METEOR{
		...
		protected IEffectData doEffect(IBeeGenome genome, IEffectData storedData, IBeeHousing housing, World world, BlockPos position) {
			if (world.rand.nextInt(100) < 1 && world.canBlockSeeSky(position)) {
				world.spawnEntityInWorld(new EntitySmallFireball(world, position.getX(), position.getY() + 64, position.getZ(), 0.0, -0.6, 0.0));
			}
			return storedData;
		}
	},
	...
	public void spawnMob(World world, BlockPos pos, String name) {
