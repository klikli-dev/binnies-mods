...
public final class RecipeRegister {
	...
	public static void postInitRecipes() {
		if (Loader.isModLoaded("ic2")) {
			GameRegistry.addRecipe(ExtraBees.honeyCrystal.getCharged(0), "#@#", "@#@", "#@#", '@', PluginApiculture.items.honeyDrop, '#', EnumHoneyDrop.ENERGY.get(1));
		}
	...
	private static void addAlvearyRecipes() {
		ItemStack alveary = PluginApiculture.blocks.getAlvearyBlock(BlockAlvearyType.PLAIN);
		Item thermionicTubes = PluginCore.items.tubes;
		Item chipsets = PluginCore.items.circuitboards;
	...
	private static void addForestryRecipes() {
		RecipeManagers.carpenterManager.addRecipe(100, Utils.getFluidFromName("water", 2000), null, new ItemStack(ExtraBees.dictionary), "X#X", "YEY", "RDR", '#', Blocks.GLASS_PANE, 'X', Items.GOLD_INGOT, 'Y', "ingotTin", 'R', Items.REDSTONE, 'D', Items.DIAMOND, 'E', Items.EMERALD);
	}
	...
	private static void addMiscItemRecipes() {
		...
		RecipeManagers.carpenterManager.addRecipe(100, Fluids.FOR_HONEY.getFluid(500), null, ExtraBeeItems.ScentedGear.get(1), " j ", "bgb", " p ", 'j', PluginApiculture.items.royalJelly, 'b', PluginCore.items.beeswax, 'p', PluginApiculture.items.pollenCluster, 'g', gear);
	}
}
