...
public class MultiFenceRecipeSizeWrapper extends BlankRecipeWrapper {
	...
	public void getIngredients(IIngredients ingredients) {
		...
		List<List<ItemStack>> types = new ArrayList<>();
		...
		List<List<ItemStack>> itemInputs = new ArrayList<>(Collections.nCopies(9, new ArrayList<>()));// ArrayList NonNullList.withSize(9, new ArrayList<>(););
		...
		for (int p = 0; p < 3; p++) {
			...
			for (int index = 0; index < 3; index++) {
				...
				if (c == '0') {
					...
					itemInputs.set(p * 3 + index,Collections.nCopies(1, null));// NonNullList.withSize(1, null));
				}
			}
		}
