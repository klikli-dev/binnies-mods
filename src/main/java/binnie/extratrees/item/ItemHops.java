...
public class ItemHops extends Item implements net.minecraftforge.common.IPlantable, IItemModelRegister {
	...
	public EnumActionResult onItemUse(ItemStack stack, EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (itemStack != null && facing == EnumFacing.UP && player.canPlayerEdit(pos, facing, itemStack) && state.getBlock().canSustainPlant(state, worldIn, pos, EnumFacing.UP, this) && worldIn.isAirBlock(pos.up())) {
			pos = pos.up();
			IBlockState blockState = crops.getStateForPlacement(worldIn, pos, facing, hitX, hitY, hitZ, 0, player, stack);
