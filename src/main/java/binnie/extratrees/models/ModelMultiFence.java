...
public class ModelMultiFence extends ModelBlockCached<BlockMultiFence, ModelMultiFence.Key> {
	...
	@Nullable
	@Override
	protected IBakedModel bakeModel(@Nonnull IBlockState state, @Nonnull Key key) {
		if (key == null) {
			return null;
		}

		AABBModelBaker baker = new AABBModelBaker();
		bakeBlock(blockClass.cast(state.getBlock()), key, baker, false);
	...
	private void bakeItemModel(BlockMultiFence block, AABBModelBaker modelBaker, Key key) {
		...
		for (int i = 0; i < 5; ++i) {
			...
			if (i == 0) {
				modelBaker.setRenderBounds(new AxisAlignedBB(0.5f - thickness, 0.0f, 0.0f, 0.5f + thickness, 1.0f, thickness * 2.0f));
			} else if (i == 1) {
				modelBaker.setRenderBounds(new AxisAlignedBB(0.5f - thickness, 0.0f, 1.0f - thickness * 2.0f, 0.5f + thickness, 1.0f, 1.0f));
			} else if (i == 2) {
				modelBaker.setRenderBounds(new AxisAlignedBB(minX, topBarMinY, minZ, maxX, topBarMaxY, maxZ));
				secondary = true;
			} else if (i == 3) {
				if (!bottomBar) {
					continue;
				}
				modelBaker.setRenderBounds(new AxisAlignedBB(minX, bottomBarMinY, minZ, maxX, bottomBarMaxY, maxZ));
				secondary = true;
			} else if (i == 4) {
				if (type.embossed) {
					minX -= SCALE * 0.9f;
					...
					modelBaker.setRenderBounds(new AxisAlignedBB(minX, minY, minZ, maxX, maxY, maxZ));
				} else {
					if (type.size != 1 || type.solid) {
						continue;
					}
					modelBaker.setRenderBounds(new AxisAlignedBB(minX, 0.5f - SCALE, minZ, maxX, 0.5f + SCALE, maxZ));
					secondary = true;
				}
			}
			modelBaker.addBlockModel(block, Block.FULL_BLOCK_AABB, null, block.getSprite(meta, secondary), 0);
		}
	}
	...
	private void bakeBlockModel(BlockMultiFence block, AABBModelBaker modelBaker, Key key) {
		...
		modelBaker.setRenderBounds(new AxisAlignedBB(minPostPos, 0.0, minPostPos, maxPostPos, POST_HEIGHT, maxPostPos));
		modelBaker.addBlockModel(block, Block.FULL_BLOCK_AABB,null, block.getSprite(meta, false), 0);
		...
		if (connectAnyX) {
			modelBaker.setRenderBounds(new AxisAlignedBB(minX, barMinY, minPostPos, maxX, barMaxY, maxPostPos));
			modelBaker.addBlockModel(block, Block.FULL_BLOCK_AABB,null, block.getSprite(meta, true), 0);
		}
		if (connectAnyZ) {
			modelBaker.setRenderBounds(new AxisAlignedBB(minPostPos, barMinY, minZ, maxPostPos, barMaxY, maxZ));
			modelBaker.addBlockModel(block, Block.FULL_BLOCK_AABB,null, block.getSprite(meta, true), 0);
		}
		if (renderBottom) {
			barMinY -= 6.0f * SCALE;
		...
		if (fenceType.embossed) {
			...
			if (connectAnyX) {
				modelBaker.setRenderBounds(new AxisAlignedBB(minX, minY, minPostPos, maxX, maxY, maxPostPos));
				modelBaker.addBlockModel(block, Block.FULL_BLOCK_AABB,null, block.getSprite(meta, false), 0);
			}
			if (connectAnyZ) {
				modelBaker.setRenderBounds(new AxisAlignedBB(minPostPos, minY, minZ, maxPostPos, maxY, maxZ));
				modelBaker.addBlockModel(block, Block.FULL_BLOCK_AABB,null, block.getSprite(meta, false), 0);
			}
		}
