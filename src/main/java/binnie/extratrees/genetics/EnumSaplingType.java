	...
	private class DefaultGermlingModelProvider implements IGermlingModelProvider {
		...
		public void registerModels(Item item, IModelManager manager, EnumGermlingType type) {
			if (type == EnumGermlingType.SAPLING) {
				germlingModel = manager.getModelLocation("extratrees", "saplings/tree_" + name().toLowerCase());
				ModelBakery.registerItemVariants(item, new ResourceLocation("extratrees", "saplings/tree_" + name().toLowerCase()));
			}
