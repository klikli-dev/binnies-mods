...
public class ControlFurtherMutation extends ControlMutation {
	...
	public ControlFurtherMutation(IWidget parent, int x, int y, IMutation mutation, float specificChance, BreedingSystem system, IAlleleSpecies firstSpecies, IAlleleSpecies secondSpecies, IAlleleSpecies resolute) {
		super(parent, x, y, mutation, specificChance, system, firstSpecies, secondSpecies);
		this.resolute = resolute;

		addTooltip(resolute.getName());
		String comb = firstSpecies.getName() + " + " + secondSpecies.getName();
		addTooltip(comb);
